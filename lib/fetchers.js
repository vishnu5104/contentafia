...
export async function getPostsForSite(domain) {
    ...
    return await unstable_cache(async () => {
        return await db
            ...
            .where(and(eq(posts.published, true), subdomain
            ? eq(sites.subdomain, subdomain)
            : eq(sites.customDomain, domain)))
...
export async function getPostData(domain, slug) {
    const subdomain = domain.endsWith(`.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}`)
        ? domain.replace(`.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}`, "")
        : null;
    return await unstable_cache(async () => {
        const data = await db
            ...
            .where(and(eq(posts.slug, slug), eq(posts.published, true), subdomain
            ? eq(sites.subdomain, subdomain)
            : eq(sites.customDomain, domain)))
        ...
        const [mdxSource, adjacentPosts] = await Promise.all([
            ...
            db
                ...
                .from(posts)
                .leftJoin(sites, eq(sites.id, posts.siteId))
                .where(and(eq(posts.published, true), not(eq(posts.id, data.id)), subdomain
                ? eq(sites.subdomain, subdomain)
                : eq(sites.customDomain, domain))),
        ]);
        return {
