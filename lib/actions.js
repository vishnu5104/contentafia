...
export const updateSite = withSiteAuth(async (formData, site, key) => {
    ...
    try {
        ...
        console.log("Updated site data! Revalidating tags: ", `${site.subdomain}.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}-metadata`, `${site.customDomain}-metadata`);
...
export const deleteSite = withSiteAuth(async (_, site) => {
    try {
        const [response] = await db
            .delete(sites)
            .where(eq(sites.id, site.id))
...
export const getSiteFromPostId = async (postId) => {
...
export const createPost = withSiteAuth(async (_, site) => {
    const session = await getSession();
    if (!session?.user.id) {
        return {
            error: "Not authenticated",
...
export const updatePost = async (data) => {
    ...
    try {
        ...
        revalidateTag(`${post.site?.subdomain}.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}-${post.slug}`);
...
export const deletePost = withPostAuth(async (_, post) => {
    try {
        const [response] = await db
            .delete(posts)
            .where(eq(posts.id, post.id))
...
export const editUser = async (formData, _id, key) => {
    const session = await getSession();
    if (!session?.user.id) {
        return {
            error: "Not authenticated",
